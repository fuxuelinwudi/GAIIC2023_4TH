...
class CPTForConditionalGeneration(CPTPretrainedModel):
    ...
    def _super_get_resized_embeddings(self,
        ...
        """
        task_old_tokens = copy.deepcopy(old_tokens)
...
class CPTForPretrain(CPTPretrainedModel):
    ...
    def forward(
        ...
        lm_loss = self.lm_loss_fct(lm_logits.float().view(-1, self.config.vocab_size), targets.view(-1))
        return lm_loss
...
class CPTCopy(CPTForConditionalGeneration):
    def __init__(self, config):
        super().__init__(config)
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(self, inputs_tensor, model_kwargs, model_input_name):
        if "encoder_outputs" not in model_kwargs:
            ...
            new_kwargs.pop('src')
            model_kwargs["encoder_outputs"] = encoder(inputs_tensor, return_dict=True, **new_kwargs)
    ...
    def prepare_inputs_for_generation(
        ...
        if 'past_key_values' in kwargs and not past:
    ...
    def forward(
        ...
        """
        origin lm head outputs
        """
        origin_lm_logits = self.lm_head(outputs[0])
        origin_lm_logits = origin_lm_logits + self.final_logits_bias.to(origin_lm_logits.device)

        """
        with pointer outputs
        """
        ...
        lm_logits = torch.stack([prob, origin_lm_logits], dim=0).mean(0)
        # lm_logits = prob
