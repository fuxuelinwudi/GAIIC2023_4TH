...
# origin CPT Decoder position embedding
...
# Nezha Relative position embedding
...
class CPTAttention(nn.Module):
    ...
    def forward(
        ...
        # self.head_dim: 64, self.num_heads: 16
        proj_shape = (bsz * self.num_heads, -1, self.head_dim)  # [bsz * num heads, seq_len, head_dim]
...
class CPTDecoder(CPTPretrainedModel):
    def __init__(self,
        ...
        # abstractive position embedding
    ...
    def forward(
        ...
        # ========================= position encoding ===============================
        # abstractive position embedding
        positions = self.embed_positions(input_shape, past_key_values_length)  # [bsz, seq_len, hidden_dim]
...
class CPTModel(CPTPretrainedModel):
    def __init__(
        ...
        # share 是 BertModel.embeddings.word_embeddings
    ...
    def get_encoder(self):
        class _Encoder(torch.nn.Module):
            ...
            def forward(self, *args, **kwargs):
                ...
                # sequence_output, all_hidden_states = self.encoder(*args, **kwargs)
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            if input_ids is None:
                token_type_ids = torch.ones_like(attention_mask).to(torch.long)
            else:
...
class CPTForConditionalGeneration(CPTPretrainedModel):
    ...
    def _super_get_resized_embeddings(self,
        ...
        """
        set new embedding's embedding for oov, remove punc
        """
        task_old_tokens = deepcopy(old_tokens)
        # task_old_tokens = [t for t in task_old_tokens if t not in punc]
    ...
    def prepare_decoder_input_ids_from_labels(self, labels: torch.Tensor, sep_token_id=None):
        if sep_token_id:
            return shift_tokens_right(labels, self.config.pad_token_id, sep_token_id)
        else:
...
# class CPTForPretrain(CPTPretrainedModel):
...
#             pdb.set_trace()
#
#         lm_loss = self.lm_loss_fct(lm_logits.float().view(-1, self.config.vocab_size), lm_labels.view(-1))
#         return lm_loss
