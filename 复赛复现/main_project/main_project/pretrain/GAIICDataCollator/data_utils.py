...
def text_segmentate(text):
    text = text.split(' ')
    idx = find_all('10', text)
    text_list = []
    left = 0
    for i in idx:
        if left != idx[-1]:
            text_list.append(' '.join(text[left:i]))
            left = i + 1
...
def pseudo_summary_f1(texts,
    ...
    for text in texts:
        if len(texts) == 0:
            continue
        try:
            ids = tokenizer.encode(text.strip())[1:-1]
        except ValueError:
            print("error, input : ", text)
            raise ValueError
        sentece_idxs_vec.append(ids)
        tokens = [tokenizer._convert_id_to_token(token) for token in ids if token != 2]
        texts_tokens.append(tokens)

    texts_tokens_rm = remove_stopwords(texts_tokens, stopwords)
    source_idxs, target_idxs = list(range(len(texts))), []
    # source_idxs, target_idxs = list(range(len(texts[0].split(' ')))), []
    # assert len(texts_tokens[0]) == len(texts[0].split(' '))
