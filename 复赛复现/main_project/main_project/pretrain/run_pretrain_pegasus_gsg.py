...
import sys
sys.path.append('.')
from modeling.modeling_pagasus import PegasusForPretrain, PegasusForConditionalGeneration, PegasusConfig
from pretrain_evaluate import Evaluate
from GAIICDataCollator.DataCollatorForGSG import GAIICDataCollator
...
parser.add_argument('--pretrain_model_path', default='./pretrain/pretrain_model/new-pegasus-base', type=str, help='')
parser.add_argument('--save_path', default='./pretrain/pretrain_model/post-pretrained-pegasus-base-gsg', type=str, help='')
...
def read_data(tokenizer):
    ...
    for i, row in tqdm(train_df.iterrows(), desc='Reading from train data', total=len(train_df)):
        ...
        all_inputs['desc'].append(desc)
        all_inputs['clinical'].append(clinical)
    ...
    for i, row in tqdm(test_b_df.iterrows(), desc='Reading from test b data', total=len(test_b_df)):
        ...
        all_inputs['desc'].append(desc)
        all_inputs['clinical'].append(clinical)
    print('>>> all data num: ', len(all_inputs['desc']))
...
def build_model():
    ...
    config = PegasusConfig.from_pretrained(user_args.pretrain_model_path)
    language_model = PegasusForConditionalGeneration.from_pretrained(
        user_args.pretrain_model_path,
        config=config
    )
    model = PegasusForPretrain(
        config=config,
...
def pretrain():
    ...
    for epoch in range(user_args.total_epoch):
        ...
        for index, data in enumerate(train_iterator):
            ...
            input_ids, attention_mask, decoder_input_ids, decoder_attention_mask, lm_labels = \
                data['input_ids'], \
                    data['attention_mask'], \
